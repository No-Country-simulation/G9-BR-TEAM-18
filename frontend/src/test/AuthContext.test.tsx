...
function TestConsumer() {
  ...
  const handleReset = () => { resetPassword('oldPass', 'newPass').catch(() => {}) }
...
describe('AuthContext', () => {
  ...
  it('login with passwordResetRequired flag stores it in user state', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ id: '1', name: 'Alice', email: 'a@a.com', password_reset_required: true }),
    })
  ...
  it('resetPassword sends request and updates user state on success', async () => {
    ...
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ id: '1', name: 'Alice', email: 'a@a.com', password_reset_required: false }),
    })
