...
class EnergyAnalysisServiceTest {
  ...
  @BeforeEach
  void setUp() {
    AnalysisRepositoryPort repo =
        new AnalysisRepositoryPort() {
  ...
  void shouldIncludeRecommendationForBadOrCritical() {
    EnergyAnalysis r = service.execute(800.0, false, 15, "Casa", 8.0);
  ...
  void shouldIncludeRecommendationForManyAppliances() {
    EnergyAnalysis r = service.execute(200.0, false, 15, "Casa", 3.0);
  ...
  void shouldIncludeRecommendationForHighHours() {
    EnergyAnalysis r = service.execute(200.0, false, 5, "Casa", 6.0);
    assertTrue(r.getRecommendations().stream().anyMatch(s -> s.contains("Distribua")));
  }

  @Test
  void specificRecommendationForExcellent() {
    EnergyAnalysis r = service.execute(60.0, false, 2, "Casa", 1.0);
