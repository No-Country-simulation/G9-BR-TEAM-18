...
public class EnergyAnalysisService implements GenerateAnalysisUseCase {
    ...
    private static final Map<String, Double> BASE_CONSUMPTION_BY_TYPE = Map.of(
            "Casa", 250.0,
            "Apartamento", 150.0,
            "Comercial", 500.0,
            "Industria", 800.0,
    ...
    private EnergyAnalysis executeRuleBased(EnergyAnalysis analysis) {
        double index = calculateInefficiencyIndex(
        ...
        if (Boolean.TRUE.equals(analysis.getPeakHourUsage())) {
            recommendations.add("Reduzir o uso de equipamentos potentes durante os horários de pico (18h às 21h).");
        }
        ...
        if (analysis.getEquipmentQuantity() > 10) {
            recommendations.add("Avalie a real necessidade de todos os equipamentos ligados simultaneamente.");
        }
