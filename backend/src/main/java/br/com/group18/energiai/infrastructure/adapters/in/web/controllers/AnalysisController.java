...
public class AnalysisController {
  ...
  public ResponseEntity<AnalysisResponseDTO> analyze(
    ...
    if (request.getAppliances() != null && !request.getAppliances().isEmpty()) {
      List<ApplianceItem> items =
          request.getAppliances().stream()
              .map(
                  a -> {
      ...
      if (request.getDailyConsumptionDistribution() != null) {
        ...
        if (dist.getAirConditioningWatts() != null)
          airConditioning = dist.getAirConditioningWatts();
        if (dist.getLightingWatts() != null) lighting = dist.getLightingWatts();
      }
    }

    EnergyAnalysis result =
        generateAnalysisUseCase.execute(
            consumptionKwh,
  ...
  public ResponseEntity<List<ApplianceTypeDTO>> listApplianceTypes() {
    List<ApplianceTypeDTO> types =
        Arrays.stream(ApplianceType.values())
            .map(
                t ->
                    new ApplianceTypeDTO(
                        t.name(),
                        t.getDisplayName(),
                        t.getMlCategory(),
  ...
  public ResponseEntity<DashboardDTO> dashboard() {
    ...
    double averageConsumptionKwh =
        all.stream().mapToDouble(EnergyAnalysis::getConsumptionKwh).average().orElse(0.0);
    double totalEstimatedCost =
        all.stream().mapToDouble(EnergyAnalysis::getEstimatedMonthlyCost).sum();
    double totalCo2EmissionKg =
        all.stream().mapToDouble(a -> a.getConsumptionKwh() * co2EmissionFactor).sum();

    TreeMap<YearMonth, Double> consumptionByYearMonth =
        all.stream()
    ...
    List<MonthlyConsumptionDTO> monthlyConsumption =
        consumptionByYearMonth.entrySet().stream()
            .map(
                entry -> {
                  String monthName =
                      entry
